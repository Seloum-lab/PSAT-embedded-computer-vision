...
nous allons ensuite lister les différents modèles que nous avons décider de comparer entre eux pour enfin spécifier quel aura 
...
Pour les modèles supportés sur $PyTorch$, nous avons utilisé l'API $thop$, pour les modèles supportés sur $ONNX$\footnote{Il a été impossible de faire cela pour le modèle MobileNet SSD supporté par ONNX}, nous avons utilisé l'API $ONNX-tools$, 
...
{Pour effectuer le banchmark, nous avons utilisé deux supports différents de systèmes embarqués, tous deux des Raspberry Pi.}
